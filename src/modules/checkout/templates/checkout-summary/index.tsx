...
const ItemPrice: React.FC<{
  ...
  return (
    <div>
      {hasReducedPrice ? (
        <>
          <p className="text-sm font-semibold text-red-primary font-price">{calculated_price}</p>
          <p className="text-xs text-grayscale-500 line-through font-price">{original_price}</p>
        </>
      ) : (
        <p className="text-sm font-semibold font-price">{calculated_price}</p>
      )}
    </div>
  )
}
