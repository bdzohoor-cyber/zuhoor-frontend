...
function ProductActions({ product, materials, disabled }: ProductActionsProps) {
  ...
  // check if the selected variant is in stock or can be backordered
  ...
  return (
    <>
      ...
      <div className="flex max-sm:flex-col gap-4">
        <NumberField
          isDisabled={
            !canAddToCart || !selectedVariant || !!disabled || isPending
          }
          value={quantity}
          onChange={setQuantity}
          minValue={1}
          maxValue={selectedVariant?.manage_inventory ? itemsInStock : 99}
          className="w-full sm:w-35 max-md:justify-center max-md:gap-2"
          aria-label="Quantity"
        />
        <Button
          onPress={handleAddToCart}
          isDisabled={!canAddToCart || !selectedVariant || !!disabled}
          isLoading={isPending}
          className="sm:flex-1"
        >
          {!selectedVariant
